...
function updateCellPlasma()
	for x=1,4 do
		...
		module.cellTemp[x] = cap(module.cellTemp[x]+(module.combustionTemp[x]*PLASMA_TO_CELL_DISIPATION), module.combustionTemp[x])
		temp = PLASMA_IDEAL_DISAPATION_TEMP-module.cellTemp[x]
		if temp>=0 then
			temp = PLASMA_DISIPATION_RATE*temp
		end
		if temp>0-PLASMA_CRITICAL_TEMP then
			module.cellPD[x] = module.cellPD[x] - PLASMA_DISIPATION_RATE
		end
		module.cellPD[x] = cap(module.cellPD[x], 0)
	end
end
function piUpdate()
